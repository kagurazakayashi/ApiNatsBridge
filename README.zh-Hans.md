...
- **令牌验证** — 可选的验证令牌校验，通过 NATS 微服务（如 NyarukoLogin UserValidator）进行，支持路径白名单与内存缓存
...
bridge:
  ...
  # 日志时间日期显示格式；Go 参照时间格式：2006-01-02 15:04:05
  #   默认值："2006-01-02 15:04:05"（YYYY-MM-DD HH:mm:ss）
  #   设为空字符串 ""：控制台不显示时间日期，但日志文件仍按照默认格式记录
  #   自定义格式如："15:04:05"（仅时间）、"01/02 15:04:05"（月/日 时:分:秒）
  #   可被路由级别 time_format 覆盖
  # time_format: ""                    # 空字符串：控制台不显示时间
...
| `time_format`      | string   | 日志时间日期显示格式；默认 `"2006-01-02 15:04:05"`（YYYY-MM-DD HH:mm:ss）；设为空字符串 `""` 时控制台不显示时间但日志文件仍使用默认格式；可按路由覆盖 |
...
| `token_fields` | []string | - | **（向后兼容，推荐改用 `return_fields`）** 旧版令牌 claims 注入配置。列出的字段从验证回覆中提取并以顶层字段注入。新部署应将令牌 claim 名称直接加入 `return_fields` |

#### `return_fields` 可选值

| 字段名        | 来源   | 说明                             |
| ------------- | ------ | -------------------------------- |
| `method`      | HTTP   | HTTP 请求方法                    |
| `path`        | HTTP   | 请求路径                         |
| `headers`     | HTTP   | 请求标头（键值对）               |
| `cookies`     | HTTP   | Cookie（键值对）                 |
| `remote_addr` | HTTP   | 直连 TCP 地址（含端口）          |
| `ip`          | HTTP   | 解析后的真实客户端 IP            |
| `params`      | HTTP   | URL 查询参数和表单参数（键值对） |
| `body`        | HTTP   | 请求体原始内容                   |
| *其他名称*    | **令牌** | **令牌 claim 字段** — 不属于以上 8 个已知字段的名称，会从令牌验证 claims 中解析并以顶层字段注入。可用 claims：`username`、`app`、`sub`、`iss`、`iat`、`nbf`、`exp`、`jti` 及通过 UserValidator `custom_claims` 写入的自定义 claims（如 `uuid`）。仅对不在 `token.path_whitelist` 中的路由生效 |

#### `return_fields` 中的令牌 Claims

启用令牌验证后，`return_fields` 中**不属于** 8 个已知 BridgeRequest 字段（`method`、`path`、`headers`、`cookies`、`remote_addr`、`ip`、`params`、`body`）的名称，会自动从令牌验证回覆的 claims 中解析并以**顶层字段**注入转发数据。
...
下游微服务可直接使用 `uuid` 来识别已认证用户，无需自行解密令牌。

令牌 claim 字段仅在以下条件满足时注入：
- 该路由不在 `token.path_whitelist` 中（令牌实际被验证）
- 令牌验证成功
- 该字段在验证回覆中存在

> **向后兼容：** 旧版 `token_fields` 配置仍可使用。其中列出的字段现在以顶层字段注入（不再包裹在 `_token` 对象中）。新部署建议直接使用 `return_fields`。
...
ApiNatsBridge 可选择性地对传入的 HTTP 请求执行验证令牌校验。启用后，它会从可配置的 HTTP 标头中提取令牌，发送至基于 NATS 的令牌验证服务（如 [NyarukoLogin UserValidator](https://github.com/kagurazakayashi/NyarukoLogin)），仅允许持有有效令牌的请求继续处理。
...
1. **客户端 → ApiNatsBridge（HTTP）**：客户端发送 HTTP 请求，并在指定的标头（默认 `Authorization`）中携带令牌，如 `Authorization: v2.local.FcG...`

2. **ApiNatsBridge → UserValidator（NATS Request）**：桥接器生成一个 UUID tag，以层级 2 格式发送 `UUID|2|令牌`（层级 2：系统＋token claims）到配置的 NATS 主题（默认 `auth.token.verify`）

3. **UserValidator → ApiNatsBridge（NATS Reply）**：验证器返回 `UUID|{"success":bool,...}` — 其中 `success:true` 表示有效（含全部标准 PASETO claims：`username`/`app`/`sub`/`iss`/`iat`/`nbf`/`exp`/`jti`），`success:false` 表示无效（含 `message` 错误消息）

4. **ApiNatsBridge → 客户端（HTTP Response）**：
   - 令牌有效：请求继续转发至后端微服务
   - 令牌无效：返回 HTTP 401，`{"error":"Unauthorized: invalid token"}`
   - NATS 错误：返回 HTTP 502，`{"error":"Bad Gateway: token verification request failed"}`
   - 缺少令牌：返回 HTTP 401，`{"error":"Unauthorized: missing authentication token"}`
...
| UserValidator → ApiNatsBridge（无效） | `UUID\|{"success":false,...}` | `550e8400-...\|{"success":false,"message":"token verification failed: ..."}` |

- `|` 为字段分隔符
- 层级 `2` 可取全部标准 PASETO claims（`username`/`app`/`sub`/`iss`/`iat`/`nbf`/`exp`/`jti`），无需额外 DB 查询
- 完整的 NATS 层级通信协议详情，请参见 [NyarukoLogin UserValidator README](https://github.com/kagurazakayashi/NyarukoLogin/blob/master/UserValidator/README.md#authtokenverify--直接-nats-介面令牌核實)
...
bridge:
  token:
    nats_subject: "auth.token.verify"   # 验证令牌用的 NATS 主题
    path_whitelist:                      # 不需要检查令牌的路径白名单
      - "/ping"
      - "/auth/login"
    header_name: "Authorization"         # 令牌所在的 HTTP 标头名称
    min_length: 10                       # 令牌最小字节长度
    max_length: 4096                     # 令牌最大字节长度
    tag_separator: "|"                   # tag 与结果之间的分隔符
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `nats_subject` | string | `auth.token.verify` | 令牌验证的 NATS 主题 |
| `path_whitelist` | []string | `[]` | 跳过令牌检查的 HTTP 路径 |
| `header_name` | string | `Authorization` | 包含令牌的 HTTP 标头名称 |
| `min_length` | int | `10` | 令牌允许的最小字节长度 |
| `max_length` | int | `4096` | 令牌允许的最大字节长度 |
| `tag_separator` | string | `\|` | 回复中分隔 tag 与结果的字符 |
...
>
> 通过 UserValidator 的 `token_claims_mapping.custom_claims` 写入令牌的自定义 claims（如 `uuid`）会自动包含在层级 2 验证回覆中，可在路由的 `return_fields` 中加入对应名称来提取（或通过旧版 `token_fields` 配置）。

### 令牌缓存

为减少 NATS 流量，已验证的令牌和最近失败的令牌结果会缓存在内存中：

- **缓存键**：完整的令牌字符串
- **缓存命中（有效）**：请求直接放行，无需 NATS 调用
- **缓存命中（无效）**：请求直接拒绝（HTTP 401）
- **缓存未命中**：执行 NATS 验证并将结果写入缓存
- **缓存淘汰**：当达到 `cache_max_entries` 上限时，清空整个缓存（冷重启策略）

### 错误代码

| HTTP 状态码 | 消息 | 原因 |
|-------------|------|------|
| 400 | `invalid token length` | 令牌长度小于 `min_length` 或大于 `max_length` |
